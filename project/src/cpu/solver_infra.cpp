    ...
    for (const auto &line : t_encodedBoards) {
        ...
        if (numbers.has_value()) {
            ...
            if (!constraints.isValid)
                spdlog::error("createBoardsConstraintsSerial() - Invalid board detected in the line {}, repeated numbers!", boardLine);
            else
...
std::optional<std::vector<CELL_TYPE>> convertLineToNumbers(const std::string_view t_line, const std::string_view t_lineNumber)
{
