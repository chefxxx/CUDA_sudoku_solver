...
{
    template <class T>
    concept cuda_pointerable_type = std::is_array_v<T> == false;

    template <class T>
    concept disallowed_known_bound = std::is_array_v<T> && std::extent_v<T> > 0;
    ...
    public:
         ...
         */
        explicit unique_ptr(pointer devPtr) noexcept : mDevPtr(devPtr), mDeleter{} {}
     ...
     */
    template <disallowed_known_bound T, class... Args>
